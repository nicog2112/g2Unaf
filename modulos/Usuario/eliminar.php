...
try {
    ...
    $sentenciaUsuario = $base_de_datos->prepare("DELETE FROM Usuarios WHERE id_usuario = ?;");
    $resultadoUsuario = $sentenciaUsuario->execute([$id]);

    if (!$resultadoUsuario) {
        throw new Exception("Error al eliminar de la tabla Usuarios");
    }
